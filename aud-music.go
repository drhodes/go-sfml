...
func NewMusicFromMemory(music []byte) (Music, error) { 
	if len(music) == 0 {
		return Music{}, errors.New("Music.CreateFromMemory received empty slice of bytes")
	}
	size := C.size_t(len(music))
	ptr := unsafe.Pointer(&music[0])
    return Music{C.sfMusic_createFromMemory(ptr, size)}, nil
}
